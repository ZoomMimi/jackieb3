...
2. **Canada side trip — an actual excursion** (2023-06-05 to 2023-08-31): a second Great Lakes excursion including a return to Canadian waters (North Channel, Ontario, Jul 14-22) — distinct from the *already-published* 2022 Georgian Bay/Canada leg. **74 candidate days** — 43 already have a `draft: true` stub, 31 have photo data but no stub generated (see full list in Decisions below). This one *is* presented with editorial framing as a side excursion in the narrative text (a return visit to Canada) — no structural/grouping change to the site, just how the writing frames it.
